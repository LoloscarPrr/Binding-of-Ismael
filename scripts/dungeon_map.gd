...
func generate(floor_number: int) -> void:
	...
	var target_count := clampi(8 + floor_index,8,12)
	_add_room(Vector2i.ZERO)
	var attempts := 0
	while rooms.size() < target_count and attempts < 900:
		attempts += 1
		var parent: Vector2i
		if _rng.randf() < 0.42 and order.size() > 2:
			parent = order[_rng.randi_range(maxi(0,order.size()-4),order.size()-1)]
		else:
...
func ordinal(cell: Vector2i) -> int:
	if not rooms.has(cell):
		return 1
	return int(rooms[cell]["ordinal"])
	...
	for cell: Vector2i in visible:
		...
		for next: Vector2i in neighbors(cell):
			if not rooms.has(next):
				continue
			var next_data: Dictionary = rooms[next]
			if not bool(next_data["discovered"]):
				continue
