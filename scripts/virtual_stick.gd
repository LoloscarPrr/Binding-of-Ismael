...
func _update_target(global_pos: Vector2) -> void:
	...
	var axis_threshold := maxf(18.0, stick_radius * 0.17)
	var x := 0.0
	var y := 0.0
...
func _draw() -> void:
	var center := size * 0.5
	var button_size := maxf(58.0, stick_radius * 0.58)
	var spacing := maxf(51.0, stick_radius * 0.54)
	var plate_radius := stick_radius * 0.92
	draw_circle(center + Vector2(0, 5), plate_radius, Color(0.015,0.018,0.022,0.34))
	draw_arc(center, plate_radius, 0.0, TAU, 48, Color(0.78,0.82,0.86,0.10), 2.0)
	...
	draw_rect(Rect2(center-Vector2(core,core)*0.5+Vector2(0,4),Vector2(core,core)),Color(0.02,0.025,0.03,0.52))
	draw_rect(Rect2(center-Vector2(core,core)*0.5,Vector2(core,core)),Color(0.10,0.11,0.12,0.74))
	draw_rect(Rect2(center-Vector2(core,core)*0.5,Vector2(core,core)),Color(0.74,0.77,0.79,0.15),false,2.0)
...
func _draw_arrow_button(center: Vector2, direction: Vector2, offset: Vector2, button_size: float, active: bool) -> void:
	...
	draw_rect(shadow_rect,Color(0.005,0.008,0.012,0.56))
	var fill := Color(0.20,0.22,0.24,0.78) if not active else Color(0.48,0.50,0.51,0.86)
	var rim := Color(0.82,0.84,0.84,0.22) if not active else Color(0.98,0.91,0.72,0.52)
	draw_rect(rect,fill)
	...
	draw_colored_polygon(arrow,Color(0.94,0.94,0.91,0.56 if not active else 0.94))
